...
/**
 * A Svelte 5 runes component that never calls `$props()` is typed as
 * `Record<string, never>`, which makes Astro reject the `client:only="svelte"`
 * directive on `<LightDarkSwitch />` with ts(2322). Declaring a real (optional)
 * prop keeps the component's props type concrete.
 */
interface Props {
	class?: string;
}
